...
* Annotation are now encoded (URI) and fail, if they are empty or contain a uppercase character
...
* `TagEncryptionHelper`, `FhirService`, `ImageResizer`, `FileService`, `NetworkConnectivityService`, `CryptoService` to Kotlin
