...
internal class RecordService(
        ...
        return Observable
                ...
                .flatMapSingle { recordId ->
                    deleteRecord(recordId, userId)
                            .doOnError { error -> failedDeletes.add(
    ...
    fun <T : DomainResource> fetchRecords(recordIds: List<String>, userId: String): Single<FetchResult<T>> {
        ...
        return Observable
                .fromCallable { recordIds }
                .flatMapIterable { recordId: List<String>? -> recordId }
                .flatMapSingle { recordId: String ->
                ...
                .filter { record -> record !is EmptyRecord<*> }
                .toList()
                .map { successfulFetches -> FetchResult(successfulFetches, failedFetches) }
    }
    ...
    fun downloadAttachments(
            ...
            type: DownloadType
                ...
                .flatMap { decryptedRecord -> downloadAttachmentsFromStorage(
                        attachmentIds,
                        userId,
                        type,
                        decryptedRecord
    ...
    private fun downloadAttachmentsFromStorage(
        ...
        if (FhirAttachmentHelper.hasAttachment(decryptedRecord.resource)) {
            ...
            .map { attachment -> attachment.also {
    ...
    fun <T : DomainResource> downloadRecord(
            ...
            userId: String
                ...
                .map { decryptedRecord -> decryptedRecord.also {
    ...
    fun <T : DomainResource> downloadRecords(
        ...
        return Observable
                .fromCallable { recordIds }
                .flatMapIterable { recordId -> recordId }
                .flatMapSingle { recordId -> downloadRecord<T>(recordId, userId)
                            .onErrorReturn { error ->
    ...
    private fun <T> decrypt(
        ...
        val commonKey: GCKey =
                if (commonKeyStored) { cryptoService.getCommonKeyById(commonKeyId) }
                else {
    ...
    fun <T : DomainResource> encryptRecord(
            record: DecryptedFhirRecord<T>
                ...
                { commonKey: GCKey ->
                        if (record.attachmentsKey == null) { null }
                        else {
    ...
    @Suppress("UNCHECKED_CAST")
    @Throws(IOException::class, DataValidationException.ModelVersionNotSupported::class)
    fun <T : DomainResource> decryptRecord(
            record: EncryptedRecord,
            userId: String
    ): DecryptedFhirRecord<T> = decrypt(
                record,
                userId
        ) { tags: HashMap<String, String>, annotations: List<String>, dataKey: GCKey, commonKey: GCKey ->
            val attachmentsKey =
                    if (record.encryptedAttachmentsKey == null) { null }
                    else {
            ...
            val resource =
                    if( record.encryptedBody == null || record.encryptedBody.isEmpty() ) { null }
                    else {
    ...
    @Throws(IOException::class, DataValidationException.ModelVersionNotSupported::class)
    fun decryptDataRecord(
            record: EncryptedRecord,
            userId: String
    ): DecryptedDataRecord = decrypt(
            record,
            userId
    ) { tags: HashMap<String, String>, annotations: List<String>, dataKey: GCKey, _: GCKey? ->
    ...
    fun <T : DomainResource> removeOrRestoreUploadData(
        ...
        return record.also { if(attachments != null && attachments.isNotEmpty()) {
