...
class AttachmentServiceTest {
    ...
    fun `Given, upload is called with Attachments, a AttachmentKey and a UserId, it returns a List of Attachments`() {
        // given
        attachment.id = "id"
        val newAttachment = mockk<Attachment>(relaxed = true)
        newAttachment.id = null
        val attachments: List<Attachment> = listOf(attachment, newAttachment)
    ...
    fun uploadAttachment_with_20MB_and_1_shouldFail() {
        ...
        try {
            val newAttachment = buildWith("newAttachment", creationDate, CONTENT_TYPE, largeFile)
            newAttachment.id = null
            val attachments: List<Attachment> = Arrays.asList(attachment, newAttachment)
            //fail("Should have thrown an exception");
        } catch (e: MaxDataSizeViolation) {
            assert(true)
        }
    }
    ...
    fun downloadAttachments_shouldReturnListOfAttachments() {
        ...
        val attachments: List<Attachment> = Arrays.asList(attachment)
        Mockito.`when`(mockFileService!!.downloadFile(attachmentKey, USER_ID, ATTACHMENT_ID)).thenReturn(Single.just(pdf))

        // when
        val subscriber: TestObserver<List<Attachment?>> = attachmentService!!.download(attachments, attachmentKey, USER_ID).test().await()
    ...
    fun downloadAttachments_shouldTNot_throw_whenInvalidHashPreview() {
        ...
        attachment!!.hash = DATA_HASH
        val attachments: List<Attachment> = Arrays.asList(attachment)
        Mockito.`when`(mockFileService!!.downloadFile(attachmentKey, USER_ID, THUMBNAIL_ID.split(ThumbnailService.SPLIT_CHAR).toTypedArray()[1])).thenReturn(Single.just(pdf))
        ...
        Truth.assertThat(a.contentType).isEqualTo(CONTENT_TYPE)
        Truth.assertThat(a.data).isEqualTo(dataBase64)
        Mockito.verify(mockFileService)!!.downloadFile(attachmentKey, USER_ID, THUMBNAIL_ID.split(ThumbnailService.SPLIT_CHAR).toTypedArray()[1])
        Mockito.verifyNoMoreInteractions(mockFileService)
    }
    ...
    fun updatingAttachments_shouldReturnListOfAttachmentsInOrder() {
        // given
        attachment!!.id = null
        val newAttachment = buildWith("newAttachment", creationDate, CONTENT_TYPE, pdf)
        val attachments: List<Attachment> = Arrays.asList(attachment, newAttachment)
        Mockito.`when`(mockFileService!!.uploadFile(attachmentKey, USER_ID, pdf)).thenReturn(Single.just(ATTACHMENT_ID))

        // when
        val subscriber: TestObserver<List<Pair<Attachment, List<String>>?>> = attachmentService!!.upload(attachments, attachmentKey, USER_ID).test().await()
    ...
    companion object {
        ...
        private const val THUMBNAIL_ID = "attachmentId#previewId"
        private const val RECORD_ID = "recordId"
        private const val USER_ID = "userId"
        private const val TITLE = "title"
        private const val CONTENT_TYPE = "contentType"
        private const val dataBase64 = "JVBERi0="
